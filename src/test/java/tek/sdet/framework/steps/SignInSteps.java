...
public class SignInSteps extends CommonUtility {
	...
	public void userFillTheSignUpInformationWithBelowData(DataTable data) {
		...
		sendText(factory.signInPage().nameInput,singUpData.get(0).get("name"));
		sendText(factory.signInPage().emailInput,singUpData.get(0).get("email"));
		sendText(factory.signInPage().passwordInput,singUpData.get(0).get("password"));
		sendText(factory.signInPage().confirmPasswordInput,singUpData.get(0).get("confirmPassword"));
